...
Salary fields remain here as optional attributes (low cardinality, high sparsity).
*/
...
select
  ...
  s.salary_year_avg,
  s.salary_hour_avg,
  null as salary_month_avg,
  md5(s.company_name) as company_id,
  md5(s.job_title_short) as title_cat_id,
  md5(s.job_schedule_type) as schedule_type_id,
  md5(s.job_via) as platform_id,
  md5(s.job_country) as country_id,
  md5(s.salary_rate) as rate_type_id,
  s.ingested_at,
